...
ВАЖНО (правило кухни, не про этот файл, но чтобы не забыть при составлении
build_rows() в order_*.py): ОМЛЕТ бывает только в 2 вариантах - обычный
(по меню дня) и безлактозный. Нельзя придумывать третий вариант омлета под
произвольное исключение (типа "без гороха/без лука") - если ни один из двух
реальных вариантов не подходит клиенту, курс заменяется на йогурт или
запечённый фрукт (что из них - смотри, что уже используется как альтернатива
для похожих исключений в том же заказе).
"""
import json
import re
...
CATEGORY_KEYWORDS = {
    ...
    "орехи": ["орех"],
    "сахар": ["сахар"],
    "грибы": ["гриб", "опят", "опёнок", "опенок"],
    "груша": ["груш"],
    "корица": ["корице", "корицы", "корица"],
    "тыква": ["тыкв"],
    # "белая рыба" - подмножество категории "рыба": треска/судак/кефаль (в отличие
    # от жирной/красной рыбы вроде лосося/форели/сельди). Своя категория нужна для
    # исключений вида "БЕЗ БЕЛОЙ РЫБЫ", отдельных от общего "БЕЗ РЫБЫ".
    "белая рыба": ["треска", "судак", "кефал"],
    # "глютен" - именно пшеница/рожь и мучные изделия, НЕ вся крупа целиком (в
    # отличие от "злаки"): гречка/рис/пшено сами по себе безглютеновые, хотя и
    # "крупа". Нужна отдельная категория для исключений "без глютена" (в отличие
    # от "без злаковых"/"без каш", которые относятся к любой крупе или к самому
    # блюду-каше).
    "глютен": ["мука", "хлеб", "булочк", "кекс", "печень", "макарон", "батон", "булгур", "кус"],
    "свинина": ["свинин"],
    "лук": ["лук"],
    "морковь": ["морков"],
    "рис": ["рис"],
...
# Категории с точным совпадением слова (а не по началу слова) - нужны там, где
# префиксное совпадение ловит другое слово. "печень" (ливер) префиксом совпадает
# со словом "печенье" (печенье начинается с "печень") - раз это разные слова с
# разным значением, сравниваем целиком.
EXACT_CATEGORY_KEYWORDS = {
    "печень": {"печень", "печени", "печенью", "печенка", "печёнка"},
}
...
def tag_composition(text):
    ...
    for category, exact_words in EXACT_CATEGORY_KEYWORDS.items():
        if any(w in exact_words for w in words):
            tags.add(category)
